...
static int hls_window(AVFormatContext *s, int last)
{
    ...
    if ((ret = avio_open2(&hls->pb, s->filename, AVIO_FLAG_WRITE|AVIO_FLAG_NONBLOCK,
...
static int hls_start(AVFormatContext *s)
{
    ...
    if ((err = avio_open2(&oc->pb, oc->filename, AVIO_FLAG_WRITE|AVIO_FLAG_NONBLOCK,
                          &s->interrupt_callback, &out_opts)) < 0){
...
static int hls_write_packet(AVFormatContext *s, AVPacket *pkt)
{
	...
	//printf("hls_write %d:%f %d: %lld %lld %lld\n", is_ref_pkt, hls->duration, can_split, pkt->pts, hls->start_pts, end_pts);
